...
private struct SchemaPropertyDesc: Decodable {
    ...
    let type: String
    let optional: Bool
    let choices: [String]?  // Hablará patch
}
...
private func buildProperty(_ prop: SchemaPropertyDesc) -> DynamicGenerationSchema.Property? {
    ...
    switch prop.type {
    case "string":
        // Hablará patch: fixed value set via anyOf
        if let choices = prop.choices, !choices.isEmpty {
            propSchema = DynamicGenerationSchema(name: prop.name + "_choice", anyOf: choices)
        } else {
