...
#        ./setup-ollama-linux.sh --model qwen3-4b
...
check_ollama_version() {
  ...
  if [[ "$current_version" != "unknown" ]]; then
    if ! version_gte "$current_version" "$MIN_OLLAMA_VERSION"; then
      ...
      log_info "${MSG_UPDATE_HINT_LINUX}"
      return 1
    fi
  fi
  return 0
...
detect_memory_bandwidth_gbps() {
  ...
  local ram_speed
  # sudo -n: nie ein Passwort-Prompt mitten im Setup; ohne Rechte bleibt die Schätzung leer
  ram_speed=$(sudo -n dmidecode -t memory 2>/dev/null | grep -i "speed:" | grep -v "Unknown" | head -1 | grep -oE '[0-9]+' | head -1 || true)
...
run_status_check() {
  ...
  if command_exists ollama; then
    ...
    if [[ "$current_version" != "unknown" ]] && ! version_gte "$current_version" "$MIN_OLLAMA_VERSION"; then
      echo -e "    ${COLOR_YELLOW}$(msg "$MSG_STATUS_UPDATE_REC_LINUX" "$MIN_OLLAMA_VERSION")${COLOR_RESET}"
    fi
  else
