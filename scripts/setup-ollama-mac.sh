...
run_status_check() {
  ...
  echo ""
  echo -e "${COLOR_CYAN}${MSG_STATUS_TITLE_MAC}${COLOR_RESET}"
  echo ""
  ...
  case "$gpu_type" in
    apple_silicon) status_ok "${MSG_STATUS_GPU_APPLE}" ;;
    nvidia)        status_ok "${MSG_STATUS_GPU_NVIDIA}" ;;
...
run_diagnose_report() {
  ...
  os_version="${os_version} $(sw_vers -productVersion 2>/dev/null || echo "${MSG_DIAGNOSE_UNKNOWN}")"
  ...
  gpu_type=$(check_gpu_available) || true
  case "$gpu_type" in
    apple_silicon) gpu_label="${MSG_DIAGNOSE_GPU_APPLE}" ;;
    nvidia)        gpu_label="${MSG_DIAGNOSE_GPU_NVIDIA}" ;;
  ...
  if [[ -r "$log_file" ]]; then
    ...
    log_output="$(msg "$MSG_DIAGNOSE_NO_LOG" "$log_file")"
  fi
...
${MSG_DIAGNOSE_SCRIPT}   setup-ollama-mac.sh v${SCRIPT_VERSION}
EOF
)
  printf '%s\n' "$report_content"
...
show_help() {
  echo ""
  echo -e "${COLOR_CYAN}Hablará Ollama Setup v${SCRIPT_VERSION} (macOS)${COLOR_RESET}"
  echo ""
...
preflight_checks() {
  ...
  if [[ "$(uname)" != "Darwin" ]]; then
    log_error "${MSG_PLATFORM_ERROR_MAC}"
    log_info "${MSG_PLATFORM_LINUX_HINT}"
    exit 4
  fi
  ...
  gpu_type=$(check_gpu_available) || true
  case "$gpu_type" in
    apple_silicon) log_success "${MSG_GPU_APPLE}" ;;
    nvidia) log_success "${MSG_GPU_NVIDIA}" ;;
    *) log_warning "${MSG_GPU_NONE}" ;;
  esac
...
start_ollama_server() {
  ...
  if command_exists ollama; then
    ...
    log_warning "${MSG_PROCESS_START_FAIL}"
    return 1
  fi
...
detect_ollama_installation() {
  ...
  # Slow fallback: check Homebrew (last resort, may take several seconds)
  if command_exists brew && run_with_timeout 15 brew list ollama &>/dev/null; then
    local brew_prefix
    brew_prefix=$(brew --prefix ollama 2>/dev/null || echo "")
    if [[ -n "$brew_prefix" && -x "${brew_prefix}/bin/ollama" ]]; then
      log_info "$(msg "$MSG_OLLAMA_BREW_FOUND" "${brew_prefix}/bin/ollama")"
...
install_ollama() {
  ...
  if command_exists brew; then
    log_info "${MSG_USING_BREW}"
    local brew_result=0
    run_with_timeout 600 brew install ollama || brew_result=$?
    if [[ $brew_result -eq 124 ]]; then
      log_error "${MSG_BREW_TIMEOUT}"
      log_info "${MSG_BREW_ALT}"
      exit 1
    elif [[ $brew_result -ne 0 ]]; then
      log_error "${MSG_BREW_FAILED}"
      log_info "${MSG_BREW_ALT}"
      exit 1
    fi
    log_success "${MSG_HOMEBREW_INSTALLED}"
